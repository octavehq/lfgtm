...
List/search/show narrowly; hydrate only selected details. For create, deduplicate and extend relevant context. For update, use exact IDs, protected-field boundaries and readback. Discover the canonical supported type and dedicated Motion APIs; never invent IDs, prefixes, mutations or timestamps.
...
Use /octave:library with the requested task, target, and output.
