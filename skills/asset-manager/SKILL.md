...
- **Workspace admins/owners can manage the team's assets.** Every asset response carries computed `canEdit` / `canManage` booleans: true for your own assets, and for any non-`only_me` asset in your workspace when your role is admin/owner. When `canManage` is true you may update files, metadata, shares, versions, stats, and transfer ownership — even for a teammate-owned asset. Never re-derive this from roles yourself; trust the fields on the response. `only_me` stays a personal drawer: `canManage` is never true for anyone but the owner, so no admin can read, edit, or share an `only_me` asset. The single way one changes hands is a transfer — which reassigns the owner outright and is recorded, rather than granting anyone a look over the owner's shoulder. Offboarding a departing member performs exactly that transfer (see Transfer Ownership).
...
1. Allowed when you own the asset, or its `canManage` reads true (workspace admin/owner, non-`only_me`). Managed skills are the one exception: they belong to the workspace itself rather than to a person, so transfer refuses them — manage those from the Skills page.
2. The target must be an active member of the asset's workspace, and must have used assets at least once (the tool returns a clear error otherwise — have them open the asset gallery or run any asset call, then retry).
