# Meeting Prep — design system & HTML architecture

The full rendering spec for `/octave:meeting-prep`: the per-section content structure, the table-card design system, status tags, tabs, the deal timeline, the brand header/footer, and the complete self-contained HTML + CSS scaffold. `SKILL.md` points here from Step 4 — **read this before generating; it is the rendering source of truth.** This renders an *internal seller doc*, so Octave entity deep-links (`/entity/{oId}`) are appropriate here (never in customer-facing assets).
